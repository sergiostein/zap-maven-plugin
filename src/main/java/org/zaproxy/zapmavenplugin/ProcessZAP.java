...
public class ProcessZAP extends AbstractMojo {
	...
	/**
	 * API token of the ZAP proxy Change this if you have set the apikey in ZAP
	 * via Options / API
	 */
	@Parameter(defaultValue = "ZAP-MAVEN-PLUGIN", required = true)
	...
	/**
	 * Set the output format type, in addition to the XML report. Must be one of
	 * "none", "json", "xml", "html".
	 */
	...
	private String createTempFilename(String prefix, String suffix) {
		...
		sb.append(dateTimeString()).append("_")
	 ...
	 *
	 * @param url
	 *            the to investigate URL
	 * @throws ClientApiException
	 */
	private void spiderURL(String url) throws ClientApiException {

		try {
			ApiResponse resp = zapClientAPI.spider.scan(apiKey, url, "-1");
			...
			while (true) {
				Thread.sleep(1000);
				progress = statusToInt((ApiResponseElement) zapClientAPI.spider
						.status(scanid));
	...
	private void scanURL(String url) throws ClientApiException {
		...
		try {
			ApiResponse resp = zapClientAPI.ascan.scan(apiKey, url, "true",
					"true", "");
			 ...
			 * System.out.println("Failed to Active Scan target : " +
			 * resp.toString(0)); return; }
			 */
			...
			while (true) {
				Thread.sleep(5000);
				progress = statusToInt((ApiResponseElement) zapClientAPI.ascan
						.status(scanid));
	 ...
	 *
	 * @param json
	 *            true for json form, false for xml format
	...
	private String getAllAlerts(final String format) throws Exception {
		...
		if (format.equalsIgnoreCase(XML_FORMAT)
				|| format.equalsIgnoreCase(HTML_FORMAT)
				|| format.equalsIgnoreCase(JSON_FORMAT)) {
	...
	public void execute() throws MojoExecutionException {
		...
		try {
			...
			proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress(
					zapProxyHost, zapProxyPort));

			if (spiderURL) {
				getLog().info(
						"Spider the site [" + targetURL + "] with apikey ["
								+ apiKey + "]");
				spiderURL(targetURL);
			} else {
			...
			if (scanURL) {
				getLog().info(
						"Scan the site [" + targetURL + "] with apikey ["
								+ apiKey + "]");
				scanURL(targetURL);
			} else {
			...
			if (reportAlerts) {
				...
				String fileName_no_extension = FilenameUtils.concat(
						reportsDirectory, fileName);

				try {
					String alerts = getAllAlerts(getAllAlertsFormat(format));
					final String fullFileName = fileName_no_extension + "."
							+ getAllAlertsFormat(format);
					FileUtils.writeStringToFile(new File(fullFileName), alerts);

					getLog().info(
							"File save in format in ["
									+ getAllAlertsFormat(format) + "]");
				} catch (Exception e) {
